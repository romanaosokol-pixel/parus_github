create or replace procedure UDO_P_REP_STAGE_KARTZATR(
...
/*        and trim(sta.NUMB) in ('0100','0101','0102','0103','0104','0105','0106','0107','0108','0109','0110',
        '0200','0201','0202','0300','0400','0401','0402','0500','0600','0700','0800','0900',
    ...
    order by numb
      ...
      if    rec.numb = '0100' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0100, rec.cost_sum);
      elsif rec.numb = '0101' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0101, rec.cost_sum);
      elsif rec.numb = '0102' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0102, rec.cost_sum);
      elsif rec.numb = '0103' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0103, rec.cost_sum);
      elsif rec.numb = '0104' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0104, rec.cost_sum);
      elsif rec.numb = '0105' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0105, rec.cost_sum);
      elsif rec.numb = '0106' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0106, rec.cost_sum);
      elsif rec.numb = '0107' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0107, rec.cost_sum);
      elsif rec.numb = '0108' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0108, rec.cost_sum);
      elsif rec.numb = '0109' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0109, rec.cost_sum);
      elsif rec.numb = '0110' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0110, rec.cost_sum);
      elsif rec.numb = '0200' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0200, rec.cost_sum);
      elsif rec.numb = '0201' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0201, rec.cost_sum);
        nPlan0201 := rec.cost_sum;
      elsif rec.numb = '0202' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0202, rec.cost_sum);
      elsif rec.numb = '0203' then -- добавляем к Основной зарплате
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0201, nPlan0201+rec.cost_sum);
      elsif rec.numb = '0300' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0300, rec.cost_sum);
      elsif rec.numb = '0400' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0400, rec.cost_sum);
      elsif rec.numb = '0401' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0401, rec.cost_sum);
      elsif rec.numb = '0402' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0402, rec.cost_sum);
      elsif rec.numb = '0500' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0500, rec.cost_sum);
      elsif rec.numb = '0600' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0600, rec.cost_sum);
      elsif rec.numb = '0700' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0700, rec.cost_sum);
      elsif rec.numb = '0800' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0800, rec.cost_sum);
      elsif rec.numb = '0900' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_0900, rec.cost_sum);
      elsif rec.numb = '1000' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1000, rec.cost_sum);
      elsif rec.numb = '1100' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1100, rec.cost_sum);
      elsif rec.numb = '1200' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1200, rec.cost_sum);
      elsif rec.numb = '1300' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1300, rec.cost_sum);
      elsif rec.numb = '1400' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1400, rec.cost_sum);
      elsif rec.numb = '1500' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1500, rec.cost_sum);
      elsif rec.numb = '1600' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1600, rec.cost_sum);
      elsif rec.numb = '1700' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1700, rec.cost_sum);
      elsif rec.numb = '1800' then
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1800, rec.cost_sum);
      elsif rec.numb = '1900' then
        --PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_1900, rec.cost_sum);
        nSum_WithotNDS := rec.cost_sum;
      end if;

      if nPrint_header = 0 then
        nPrint_header := 1;
        PRSG_EXCEL.CELL_VALUE_WRITE(C_nPlan_trud,      rec.LAB_STAG);
        PRSG_EXCEL.CELL_VALUE_WRITE(C_sRep_yare,  'на '||rec.nDoc_Yare ||' г.');
        PRSG_EXCEL.CELL_VALUE_WRITE(C_sWork_name,      rec.Work_name);
    --    PRSG_EXCEL.CELL_VALUE_WRITE(C_sOKP_OKPD2_CODE, rec.);
        PRSG_EXCEL.CELL_VALUE_WRITE(C_sSTAGE_NAME, 'Этап '|| rec.Stage_numb);
...
--        PRSG_EXCEL.CELL_VALUE_WRITE(C_sFact_yare,      rec.nDoc_Yare||' г.');
        ...
        sAgentName := REC.AGENT_NAME;
        --if ('' = sStageNumb) then 
          sStageNumb := trim(rec.Stage_numb);
        --else sStageNumb := sStageNumb || ',' || trim(rec.Stage_numb);
        --end if;
        sWork_name := REC.Work_name;
        dDateStart := REC.DATESTART;
        dDateEnd   := REC.DATEEND;
        nPlan0201 := 0;
      end if;
