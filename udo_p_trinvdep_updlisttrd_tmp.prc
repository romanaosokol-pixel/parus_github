create or replace procedure UDO_P_TRINVDEP_UPDLISTTRD_TMP
(
...
begin
  ...
  for rdp in ( select ARC.RN as ARC_RN,
               ...
               where ARC.MODIFDATE between dBEG and dEND
                 ...
                 and exists(select null from TRANSINVDEPTSPECS TDS
                                   where TDS.RN = ARC.TABLERN and TDS.PRN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT))
              ...
              select null as ARC_RN,
               ...
               where AR.MODIFDATE between dBEG and dEND
                 ...
                 and exists(select null from TRANSINVDEPTSPECS TDS
                                   where TDS.RN = AR.TABLERN and TDS.PRN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT))
                 and exists (select null
                        from UPDATELIST_DETAIL DA
                       where DA.PRN = AR.RN
