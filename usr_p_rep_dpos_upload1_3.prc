create or replace procedure usr_p_rep_dpos_upload1_3
/*
Процедура к отчёту "Выгрузка по спецификациям заказов подразделений 1.3"
...
begin
  ...
  udo_pkg_excel_report_xml.p_add_cell_value_string(psstyle => null, psvalue => '#Автор Заказа');
  ...
  for c in (
            select nvl(po.release_date, dpo.release_date)                                         as release_date
                  ...
                  ,(
                    select dpv.str_value
                      from docs_props_vals dpv
                     where dpv.docs_prop_rn = 8027721
                       and dpv.unit_rn = po.head_rn
                   )                      as po_head_1C_ord
                  ...
                  ,UDO_F_DEPARTMENTORD_AUTHOR(nrn => dpo.rn)                                          as dpo_author -- Автор заказа
                  ,(select str_value
                      from V_DOCS_PROPS_VALS_SHADOW
                      where DOCS_PROP_RN = 180597323
                        and unit_rn = dpos.NOMEN)                                                     as umts_buyer
                  ...
                  ,substr(      get_doctypes_code_id(nflag_smart => 1, nrn => bp.head_type)
                         ,instr(get_doctypes_code_id(nflag_smart => 1, nrn => bp.head_type), '_') +1
                         )                                      as bp_head_type
              ...
              from (
                  ...
                  ,(
                    select dl_1.out_document  as dl_1_out_document
                          ,h.rn               as head_rn
                          ,h.ord_pref         as head_pref
                          ,h.ord_numb         as head_numb
...
--                  ,(select to_date('20.01.2025', 'dd.mm.yyyy') as bg, to_date('06.02.2025', 'dd.mm.yyyy') as en from dual ) d
             where dpos.prn         = dpo.rn
               ...
               and (dpos.sumts_double != 'Да' or dpos.sumts_double is null)
...
--and (po.head_rn = 116124197 or user != 'STEPANOV_MV')
  ...
  loop
    ...
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.dpo_author); -- Автор заказа
    nCountColumn := nCountColumn + 1;
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.umts_buyer); -- УМТС. Ответственный закупщик
    /* Конец строки */
    udo_pkg_excel_report_xml.p_row_end;
  end loop;
