create or replace procedure usr_p_rep_stpc_barcode1
/*
24/09/2022 Степанов М.
Процедура для отчёта "Места хранения со штрих-кодами (формат для склада ЭРИ)".
*/
(
 nIDENT             in number
)
...
begin
  ...
  for c in ( select t.rn, udo_f_stplcells_sfullcell(ncompany => t.company, nrn => t.rn) as sname
               from selectlist  sl
                   ,stplcells   t
