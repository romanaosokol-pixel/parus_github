create or replace procedure USR_P_TRANSINVCUST_0330212
(
 nCOMPANY            in number      
,nIDENT              in number      
...
begin
  ...
  prsg_excel.cell_value_write( scell_name => scell||'0004', scell_value => usr_pkg_agnlist.agnlist_get_str_details(nflagsmart  => 1
                                                                                                                 ...
                                                                                                                 ,ddate       => rDoc.ddate
                                                                                                                 ,sparam_list => '1;2;3') );
  prsg_excel.cell_value_write( scell_name => scell||'0005', scell_value => rAgnFiFoAgn.orgcode );
  /* Поставщик */
  prsg_excel.cell_value_write( scell_name => scell||'0006', scell_value => usr_pkg_agnlist.agnlist_get_str_details(nflagsmart  => 1
                                                                                                               ...
                                                                                                               ,ddate       => rDoc.ddate
                                                                                                               ,sparam_list => '1;2;3') );
  prsg_excel.cell_value_write( scell_name => scell||'0007', scell_value => rSellerAgn.orgcode );
  /* Плательщик */
  prsg_excel.cell_value_write( scell_name => scell||'0008', scell_value => usr_pkg_agnlist.agnlist_get_str_details(nflagsmart  => 1
                                                                                                              ,ragn        => rPayerAgn
                                                                                                              ,ragnacc     => rPayerAcc
                                                                                                              ,ddate       => rDoc.ddate
                                                                                                              ,sparam_list => '1;2;3') );
  prsg_excel.cell_value_write( scell_name => scell||'0009', scell_value => rPayerAgn.orgcode );

  /* Основание */
  sTMP := null;
  sTMP := pkg_document.make_number( ndoc_type => rContracts.doc_type
                                   ,sdoc_pref => null
                                   ,sdoc_numb => nvl( rContracts.ext_number
                                                    , pkg_document.make_number(sdoc_pref => rContracts.doc_pref, sdoc_numb => rContracts.doc_numb) )
                                   ,ddoc_date => rContracts.doc_date );
  if nStagesCount > 1 then
    sTMP := strcombine( sTMP, trim(rStages.numb), ', Этап ');
  end if;
  sTMP := strcombine( sTMP, pkg_document.make_number( ndoc_type => rAccDoc.ndoctype /*sdoc_type => 'Счёт'*/
                                                     ,sdoc_pref => null
                                                     ,sdoc_numb => nvl( rAccDoc.sext_numb, trim( rAccDoc.snumber) )
                                                     ,ddoc_date => rAccDoc.ddate )
                    , ', ' );
  sTMP := strcombine( sTMP, rGovCntrId.code, ', ИГК ');
  prsg_excel.cell_value_write( scell_name => scell||'0010', scell_value => sTMP );
  ...
  /* Номер накладной */
  prsg_excel.cell_value_write( scell_name => scell||'0013', scell_value => trim(rDoc.snumber) );
  /* Дата накладной */
  prsg_excel.cell_value_write( scell_name => scell||'0014', scell_value => to_char(rDoc.ddate, 'dd.mm.yyyy'));
  ...
  for c in (
            select dnm.nomen_name     as sNomen_Name
                  ...
                  ,sum(np.quant)      as nQuant_Pack
                  ,sum(t.summwithnds) as nSumm_With_NDS
  ...
  /* Расходные накладные на возврат поставщикам */
            select dnm.nomen_name     as sNomen_Name
                  ...
                  ,f_get_taxis_value( nflag_smart  => 1
                                     ...
                                     ,ddate        => rDoc.ddate
                                     ,nkind        => 1 ) as nTax_Value
                  ,round(t.summtax / t.quant, 2) /* для разделения спецификаций по ценам расчитываем её от суммы и округляем */
                  ,listagg(usr_pkg_rlarticles.rlarticles_get_short_numb(snumb => ra.code), ';') within group(order by ra.code)        as sArticle_List
                  ,listagg(gp.sernumb, ';') within group(order by gp.sernumb)                                                         as sSernumb_List
                  ,sum(t.quant)       as nQuant
                  ,sum(t.quantalt)    as nQuant_Alt
                  ,sum(np.quant)      as nQuant_Pack
                  ,sum(t.summtax)     as nSumm_With_NDS
              from rinvtosupspecs  t
                  ...
                  ,goodsparties      gp
                  ,goodssupply       gs
             where sunitcode      = 'ReturnInvoicesToSuppliers'
               and t.prn            = rDoc.nrn
               and t.nommodif       = nm.rn
               and nm.prn           = dnm.rn
               and t.nomnmodifpack  = nmp.rn(+)
  ...
  loop
    ...
    if nDIVPAGE = 1 then
      ...
      if nNEWPAGE = 1 then
        ...
        if nITPAGE = 1 then
          ...
          if nQUANT_IT <> 0 then
            prsg_excel.cell_value_write( scell_name => scell||'0051', icell_index_x => 0, icell_index_y => iLINE, scell_value => nQUANT_IT );
          end if;
          if nSUMM_IT <> 0 then
            prsg_excel.cell_value_write( scell_name => scell||'0052',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM_IT );
          end if;
          if nNDS_IT <> 0 then
           prsg_excel.cell_value_write( scell_name => scell||'0053',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nNDS_IT );
          end if;
          if nSUMM_NDS_IT <> 0 then
            prsg_excel.cell_value_write( scell_name => scell||'0054',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM_NDS_IT );
          end if;
          nPASK_IT     := 0;
          nQUANT_IT    := 0;
          nSUMM_IT     := 0;
        ...
        if nNUMB_DATA_PAGE = 1 and nNUMB_LINES_FIRST1 = 0 then
          ...
          prsg_excel.line_page_break( scurrent_line_name => SH1_HEAD );
          if nNUMB_DATA_PAGE = 1 then
            prsg_excel.cell_value_write( scell_name => SH1_PAGE2,  icell_index_x => 0, icell_index_y => iLINE, scell_value => 'Оборотная сторона формы № ТОРГ-12' );
          end if;
        end if;
        nNUMB_LINES_PAGE := 1;
    ...
    prsg_excel.cell_value_write( scell_name => scell||'0030',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nREC_QUANT );

    /* Товар_наим */
    sTMP := strcombine( c.sNomen_Name, c.sNote, ' '); 
    prsg_excel.cell_value_write( scell_name => scell||'0031',  icell_index_x =>  0, icell_index_y => iLINE, scell_value => sTMP );
    ...
    if c.sArticle_List is null and c.sSernumb_List is null then
      p_exception(0, 'В спецификации не заполнены ни номер изделия, ни серия номенклатуры.%s%s'
                 ,scell||'0031' ||', кол-во: '||c.nQuant||', сумма: '||c.nSumm_With_NDS );
    else
      sTMP := null;
    ...
    prsg_excel.cell_value_write( scell_name => scell||'0032',  icell_index_x => 0, icell_index_y => iLINE, scell_value => sTMP );
    ...
    prsg_excel.cell_value_write( scell_name => scell||'0033',  icell_index_x => 0, icell_index_y => iLINE, scell_value => sMEAS );
    ...
    prsg_excel.cell_value_write( scell_name => scell||'0034',  icell_index_x => 0, icell_index_y => iLINE, scell_value => sMEAS );
    ...
    nQUANT := nvl( nQUANT, 0 );
    if nQUANT <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0038',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nQUANT );
    end if;
    ...
    /* Сумма_без_НДС */
    nSUMM := pkg_dictaxis_calc.f_get_value(nident => 0) * /*rTRINC.CURBASE*/ 1;
    nSUMM := nvl(nSUMM,0);
    if nSUMM <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0040',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM );
    end if;
    ...
    nNDS := nvl(nNDS,0);
    if nNDS <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0042',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nNDS );
    end if;
    ...
    if nSUMM_NDS <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0043',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM_NDS );
    end if;
  ...
  if nREC_QUANT > 0 then
    ...
    if nQUANT_ALL <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0056',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nQUANT_ALL );
    end if;
    if nSUMM_ALL <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0057',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM_ALL );
    end if;
    if nNDS_ALL <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0058',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nNDS_ALL );
    end if;
    if nSUMM_NDS_ALL <> 0 then
      prsg_excel.cell_value_write( scell_name => scell||'0059',  icell_index_x => 0, icell_index_y => iLINE, scell_value => nSUMM_NDS_ALL );
    end if;
