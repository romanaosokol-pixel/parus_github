create or replace procedure usr_p_payaccinspclc_cre2(nrn in payaccinspec.rn%type) is
  ...
  n_fl number(1); /*ѕризнак наличи€ калькул€ции у спецификации счета*/

begin

  /*»справление/ƒобавление калькул€ции —“–ќ » —ѕ≈÷»‘» ј÷»» вход€щего счета, значени€ми по умолчанию */

  /*1 ќтберем все калькул€ции одной строки спецификации вход€щего счета */

  for spe in (select ps.rn
                    ,ps.company com
                    ,udo_f_payaccin_faceacc_article(nrn => ps.prn) scost_article /*—татью затрат берем из лицевого счета */
  ...
  loop
    ...
    loop
      ...
      usr_p_payaccinspclc_update_ini(nrn                => cl.rn
                                    ...
                                    ,scost_article      => spe.scost_article
