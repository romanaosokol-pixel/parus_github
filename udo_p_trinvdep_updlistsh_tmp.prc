create or replace procedure UDO_P_TRINVDEP_UPDLISTSH_TMP
(
...
begin
  ...
  rDET.Ident     := nIDENT;
  rDOC.Ident     := nIDENT;
  -- выбираем удаления спецификаций или изменения количества в спецификации за период
  for rdp in ( select ARC.RN as ARC_RN,
               ...
               where ARC.MODIFDATE between dBEG and dEND
                 ...
                 and (exists(select null from DOCLINKS L, FCDELIVSH SH
                                    where L.OUT_DOCUMENT = ARC.TABLERN and L.OUT_UNITCODE = 'GoodsTransInvoicesToDeptsSpecs'
                                      and L.IN_DOCUMENT = SH.RN and L.IN_UNITCODE = 'CostDeliverySheets'
                                      and SH.RN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT)) or
                      exists(select null from DOCLINKS L, FCDELIVSH SH
                                    where L.OUT_DOCUMENT = to_number(trim(substr(ARC.NOTE, 5, instr(ARC.NOTE, ',')-5)))
                                      and L.OUT_UNITCODE = 'GoodsTransInvoicesToDepts'
                                      and L.IN_DOCUMENT = SH.RN and L.IN_UNITCODE = 'CostDeliverySheets'
                                      and SH.RN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT)))
                 and (exists (select null
                        from UPDATELIST_DETAIL DA
                       where DA.PRN = ARC.RN
                         and DA.COLUMN_NAME = 'QUANT') or
                      ARC.OPERATION = 'D')
              union all
              select null as ARC_RN,
               ...
               where AR.MODIFDATE between dBEG and dEND
                 ...
                 and (exists(select null from DOCLINKS L, FCDELIVSH SH
                                    where L.OUT_DOCUMENT = AR.TABLERN and L.OUT_UNITCODE = 'GoodsTransInvoicesToDeptsSpecs'
                                      and L.IN_DOCUMENT = SH.RN and L.IN_UNITCODE = 'CostDeliverySheets'
                                      and SH.RN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT)) or
                      exists(select null from DOCLINKS L, FCDELIVSH SH
                                    where L.OUT_DOCUMENT = to_number(trim(substr(AR.NOTE, 5, instr(AR.NOTE, ',')-5)))
                                      and L.OUT_UNITCODE = 'GoodsTransInvoicesToDepts'
                                      and L.IN_DOCUMENT = SH.RN and L.IN_UNITCODE = 'CostDeliverySheets'
                                      and SH.RN in(select DOCUMENT from SELECTLIST where IDENT = nIDENT)))
                 and (exists (select null
  ...
  -- расходные накладные по изменениям
  ...
  for rtm in (select *
    ...
    -- когда последний раз сняли отработку
