create or replace procedure usr_p_payaccinsall_insert_ini(nrn                in payaccin.rn%type
                                                          ,nprn               in number
                                                          ,ncompany           in number
...
begin
/*Валидатор на открытие формы для процедуры заведение калькуляции по всем строкам спецификации из заголовка входящего счета */
 

  usr_p_payaccinspclc_finpl_def(nrn             => nrn
                               ,ncompany        => ncompany
                               ,speriod         => speriod
  ...
  with sq as
   (select t.quant       q
    ...
    select -cl.quant_fact
          ,-cl.cost_fact * cl.quant_fact
...
/* Найдем ШПЗ */
  ...
  cost_year  := usr_f_finplan_art_cost(nrn, 0);
  --- if user = 'GOR' then P_exception(0, nrn||' ' ||cost_year||' '||limit_year); end if;
