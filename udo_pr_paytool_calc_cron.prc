create or replace procedure UDO_PR_PAYTOOL_CALC_CRON(
       nCOMPANY0    in number  -- Организация
--       nIDENT      in numeric -- Выбранные записи
)
is
-- Считаем остатки на предыдущий день. Обнуленные ранее счета не трогаем. Их дата показывает когда они обнулились.
-- В таблицу заносится Дата вычисления, а на 1 день меньше. Принимаем, что это остатки на конец дня.

/* UDO_PR_PAYTOOL_CALC(nCOMPANY   => nCOMPANY,
                      nIDENT     => nIDENT,
                      dCALC_DATE => sysdate-1);*/
                      
  dCALC_DATE date := sysdate-1;
...
--p_exception(0,'sStartDate ' || sStartDate || '; dCALC_DATE ' || dCALC_DATE);
  ...
  for rec in (
    select fin.rn tool_rn, nvl(fin.currency, 91318) currency, fin.payer_acc acc_rn,-- dic.doc_rn, 
      ...
      from FINPAYTOOL fin, DICANLS dic, ANLREMNS anl
     where fin.payer_acc = dic.doc_rn and dic.rn = anl.analytic1
    ...
    select fin.rn tool_rn, nvl(fin.currency, 91318) currency, fin.payer_acc acc_rn,-- dic.doc_rn, 
      ...
      from FINPAYTOOL fin, DICANLS dic, ANLREMNS anl
     where dic.doc_rn (+)= fin.payer_acc  and anl.analytic1 (+)= dic.rn
       and anl.remn_date is null
--order by currency desc
  ) loop
/*if (7614009 = rec.acc_rn) then
p_exception(0,'Exist: tool_rn: ' || rec.tool_rn || '; nJUR_PERS: ' || nJUR_PERS || '; dCALC_DATE: ' || dCALC_DATE); 
      ...
      begin  -- Дебет с даты подсчета остатков
        ...
        where bank.agent_to_acc = rec.acc_rn and bank.bank_docdate between sStartDate and dCALC_DATE -- от '01-JAN-2021'
      ...
      begin  -- Кредит с даты подсчета остатков
        ...
        where bank.agent_from_acc = rec.acc_rn and bank.bank_docdate between sStartDate and dCALC_DATE -- от '01-JAN-2021'
...
--p_exception(0,'acnt_remn_sum: ' || rec.acnt_remn_sum || '; nStartDeb: ' || nStartDeb || '; nStartCred: ' || nStartCred); 
    ...
    if (rec.currency is null or '' = rec.currency) then nCurrency := 91318; -- RUB
    else nCurrency := rec.currency;
    end if;

    if nRNTMP.RN is null then
      if (rec.acnt_remn_sum + nStartDeb - nStartCred != 0) then
        --p_exception(0,'Exist: currency "' || nCurrency || '" tool_rn: ' || rec.tool_rn || '; nJUR_PERS: ' || nJUR_PERS || '; dCALC_DATE: ' || dCALC_DATE); 
