create or replace procedure UDO_P_TRINVDEPT_FORM_MAKERTLST
(
...
begin
  ...
  if sATTRIB is null then
    ...
    /* Каталог */
    FIND_ROOT_CATALOG(nCOMPANY, 'CostRouteLists', nCRN);
    ...
    /* Лицевой счет */
    begin
    select GET_FACEACC_NUMB_ID(1, t.faceacc)
      into sFACEACC
      from TRANSINVDEPT t
      where t.rn      = nRN
...
end UDO_P_TRINVDEPT_FORM_MAKERTLST;
/
