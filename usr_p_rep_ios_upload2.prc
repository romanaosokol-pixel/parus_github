create or replace procedure usr_p_rep_ios_upload2
/*
Процедура к отчёту "Выгрузка по спецификациям приходных ордеров 2"
...
begin
  ...
  udo_pkg_excel_report_xml.p_add_cell_value_string(psstyle => null, psvalue => 'Приходный ордер');
  ...
  for c in (
            select (
                    select min(pkg_document.make_number(t.ord_doctype, t.ord_pref, t.ord_numb, t.ord_date))
                      from doclinks   dl_1
                          ,doclinks   dl_2
                          ,doclinks   dl_3
                          ,deliveryord  t
                     where dl_1.out_document  = io.rn
                       and dl_1.in_document   = dl_2.out_document
                       and dl_2.in_document   = dl_3.out_document
                       and dl_3.in_document   = t.rn
                   ) dlo
                  ,(
                    select min(pkg_document.make_number(t.doc_type, t.doc_pref, t.doc_numb, t.doc_date))
                      from doclinks   dl_1
                          ,doclinks   dl_2
                          ,payaccin   t
                     where dl_1.out_document  = io.rn
...
--and ios.rn = 82090274
  ...
  udo_pkg_excel_report_xml.p_save_result(psfilename => 'Выгрузка по спецификациям приходных ордеров 2.xls'
                                        ,pnident    => nIDENT_PROCESS);
  /* Освобождаем ресурсы отчета */
  udo_pkg_excel_report_xml.p_finalize;

end USR_P_REP_IOS_UPLOAD2;
/
