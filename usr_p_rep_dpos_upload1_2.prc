create or replace procedure usr_p_rep_dpos_upload1_2
/*
Процедура к отчёту "Выгрузка по спецификациям заказов подразделений 1.2"
...
/*  udo_pkg_excel_report_xml.p_add_cell_value_string(psstyle => null, psvalue => 'Заказ подразделения. Добавил');*/
  ...
  for c in (
            select nvl(po.release_date, dpo.release_date)                                                           as release_date 
                  ,pkg_document.make_number(sdoc_pref => po.head_pref, sdoc_numb => po.head_numb)                   as po_head_numb
                  ,po.head_date                                                                                     as po_head_date
                  ,( select str_value from docs_props_vals where docs_prop_rn = 8027721 and unit_rn = po.head_rn )  as po_head_1C_ord
                  ,po.head_state_date                                                                               as po_head_state_date
                  ,po.head_state_spers                                                                              as po_head_state_spers
                  ,dnm.nomen_code
                  ,dnm.nomen_name
                  ,pkg_document.make_number(sdoc_pref => dpo.ord_pref, sdoc_numb => dpo.ord_numb)                                 as dpo_head_numb
                  ,dpo.ord_date                                                                                                   as dpo_head_date
                  ,usr_pkg_departmentord.departmentord_get_status_name(nord_state => dpo.ord_state)                               as dpo_state
                  ,usr_pkg_updatelist.updatelist_get_last_authid(nflagsmart => 1, nrn => dpo.rn, soperation => 'I')               as dpo_ul_auth
                  ...
                  ,( select max(h.work_date)
                       from buyplandirspref  sr
                           ,buyplandirsp     s
                           ,buyplandir       h
                      where s.rn     = sr.prn
                        and h.rn     = s.prn
                        and dpos.rn  = sr.deptordsp
                        and sr.quantplan_bef is not null 
                        and sr.quantplan_aft != 0 ) as bpd_date_0
                  ,( select max(h.work_date)
                       from buyplandirspref  sr
                           ,buyplandirsp     s
                           ,buyplandir       h
                      where s.rn = sr.prn
                        and h.rn = s.prn
                        and dpos.rn  = sr.deptordsp
                        and sr.quantplan_aft != 0 ) as bpd_date_1
                  ,( select max(h.work_date)
                       from buyplandirspref  sr
                           ,buyplandirsp     s
                           ,buyplandir       h
                      where s.rn = sr.prn
                        and h.rn = s.prn
                        and dpos.rn  = sr.deptordsp
                        and sr.quantplan_aft = 0 ) as bpd_date_2
                  ,substr(      get_doctypes_code_id(nflag_smart => 1, nrn => bp.head_type)
                         ,instr(get_doctypes_code_id(nflag_smart => 1, nrn => bp.head_type), '_') +1) as bp_head_type
                  ,pkg_document.make_number(sdoc_pref => bp.head_pref, sdoc_numb => bp.head_numb)     as bp_head_numb
                  ,bp.head_date                                                                       as bp_head_date
                  ,bp.incl_date                                                                       as bp_incl_date
                  ,bp.quant                                                                           as bp_quant
                  ,( select doccode from doctypes where rn = pai.head_type )                          as pai_head_type
                  ,pkg_document.make_number(sdoc_pref => pai.doc_pref, sdoc_numb => pai.doc_numb)     as pai_head_numb
                  ,pai.head_date                                                                      as pai_head_date
                  ,pai.quant                                                                          as pai_quant
                  ,pai.ndelivdays                                                                     as pai_ndelivdays
                  ,( select agnname from agnlist where rn = pai.supplier )                                            as pai_agnname
                  ,usr_pkg_updatelist.updatelist_get_last_authid(nflagsmart => 1, nrn => pai.h_rn, soperation => 'I') as spai_ul_auth
                  ,( select init_authid from clnevents where linked_rn = pai.h_rn )                                   as spai_ce_auth
                  ,( select min(pn.pay_date)
                       from doclinks dl
                           ,paynotes pn
                      where dl.in_document  = pai.h_rn
                        and dl.out_document = pn.rn
                        and pn.signplan     = 0 )                                                 as pai_pay_date
                  ,( select doccode from doctypes where rn = dlo.head_type )                      as dlo_head_type
                  ,pkg_document.make_number(sdoc_pref => dlo.ord_pref, sdoc_numb => dlo.ord_numb) as dlo_head_numb
                  ,dlo.head_date                                                                  as dlo_head_date
                  ,( select agnname from agnlist where rn = dlo.agent )                           as dlo_agnname
                  ,usr_pkg_updatelist.updatelist_get_last_authid(nflagsmart => 1, nrn => dlo.h_rn, soperation => 'I') as sdlo_ul_auth
                  ,( select pers_authid from clnpersons where pers_agent = dlo.acc_agent )                            as sdlo_pers_auth
                  ,pkg_document.make_number(sdoc_pref => io.head_pref, sdoc_numb => io.head_numb) as io_head_numb
                  ,io.head_date                                                                   as io_head_date
                  ,io.quant                                                                       as io_quant
              from ( select t.*
                           ,( select str_value from docs_props_vals where docs_prop_rn = 22244761 and unit_rn = t.rn ) as sumts_double
                       from departmentords t ) dpos
              join departmentord  dpo on dpo.rn = dpos.prn
              join dicnomns       dnm on dnm.rn = dpos.nomen
              left join ( select dl.out_document
                                ,t.rn               as head_rn
                                ,t.ord_pref         as head_pref
                                ,t.ord_numb         as head_numb
                                ,t.ord_date         as head_date
                                ,r.authid           as head_state_spers
                                ,trunc(r.reg_date)  as head_state_date
                                ,nvl( ( select date_value
                                          from docs_props_vals
                                         where docs_prop_rn = 113738795
                                           and unit_rn = t.rn )
                                     , t.release_date ) - 91 as release_date
                            from productord t
                            join doclinks   dl_1  on dl_1.in_document  = t.rn 
                                                 and dl_1.out_unitcode = 'CostProductExpenseActs'
                            join doclinks   dl    on dl.in_document    = dl_1.out_document
                            left join( select a.tablern, a.authid, max(a.reg_date) as reg_date
                                         from ( select *
                                                  from ( select ul.tablern, ul.authid, uls.reg_date
                                                           from updatelist         ul
                                                           join updatelist_detail  uls 
                                                             on uls.prn          = ul.rn
                                                         ...
                                                         select ula.tablern, ula.authid, ulas.reg_date
                                                           ...
                                                           join updatelist_detail_arc  ulas
                                                             on ulas.prn         = ula.rn
                                                            and ulas.column_name = 'ORD_STATE'
                                                            and ulas.num_value   = 1
                                                         order by reg_date desc ) e ) a
                                        group by a.tablern, a.authid ) r
                              on t.rn = r.tablern ) po
                on po.out_document = dpos.prn 
              /* План закупок */
              left join ( select dl.in_document     as dl_in_document
                                ,s.nomen
                                ,h.doctype          as head_type
                                ,h.pref             as head_pref
                                ,h.numb             as head_numb
                            ...
                            from doclinks         dl
                            join buyplanesp       s  on s.rn   = dl.out_document
                            join buyplane         h  on s.prn  = h.rn
                            join buyplanespref    p  on p.prn  = s.rn
                            join departmentordps  pd on pd.prn = p.deptordsp
                           where s.quant_plan != 0
                          group by dl.in_document, s.nomen, pd.prn, h.doctype, h.pref, h.numb, h.docdate, s.incl_date ) bp
                on bp.dl_in_document = dpos.rn 
               and bp.pd_prn         = dpos.rn
              /* Вх.счета */
              left join ( select sce.departmentordsp
                                ,h.rn            as h_rn
                                ,s.nomen
                                ,sc.faceaccount  as sc_faceaccount
                                ,h.doc_type      as head_type
                                ,h.doc_pref      as doc_pref 
                                ,h.doc_numb      as doc_numb 
                                ,h.doc_date      as head_date
                                ,sum(sce.quant)  as quant
                                ,max( ( select num_value
                                          from docs_props_vals
                                         where docs_prop_rn = 7551156
                                           and unit_rn = s.rn ) ) as nDelivDays
                                ,h.supplier
                            from payaccinspclc_ex  sce
                            join payaccinspclc     sc  on sc.rn = sce.prn
                            join payaccinspec      s   on s.rn  = sc.prn
                            join payaccin          h   on h.rn  = s.prn
                          group by sce.departmentordsp, h.rn, s.nomen, sc.faceaccount, h.doc_type, h.doc_pref, h.doc_numb, h.doc_date, h.supplier ) pai
                on pai.departmentordsp = dpos.rn
              /* Заказы поставщикам */
              left join ( select bpsp.deptordsp
                                ,h.rn           as h_rn
                                ,h.ord_doctype  as head_type
                                ,h.ord_pref     as ord_pref
                                ,h.ord_numb     as ord_numb
                                ,h.ord_date     as head_date
                                ,h.agent        as agent
                                ,h.acc_agent    as acc_agent
                            from deliveryord  h
                            join deliveryords s                      on s.prn       = h.rn
                            join udo_uzd_03_buyplanesp_cntr_doc uzd  on uzd.doc_rn  = s.rn
                            join buyplanespref                  bpsp on bpsp.rn     = uzd.rn_ref
                            join buyplanesp                     bps  on bps.rn      = bpsp.prn
                            join buyplane                       bp   on bp.rn       = bps.prn
                          group by bpsp.deptordsp, h.rn, h.ord_doctype, h.ord_pref, h.ord_numb, h.ord_date, h.agent, h.acc_agent ) dlo
                on dlo.deptordsp = bp.pd_prn
              /* Приходные накладные */
              left join ( select dl.in_document      
                                ,nm.prn             as nomen
                                ,h.rn               as h_rn
                                ,sc.faceaccount     as sc_faceaccount
                                ,h.pref             as head_pref
                            ...
                            from ininvoices       h  
                            join ininvoicesspecs  s   on s.prn  = h.rn
                            join ininvoicesspc    sc  on sc.prn = s.rn
                            join nommodif         nm  on nm.rn  = s.modif
                            join doclinks         dl  on dl.out_document = h.rn
                                                     and dl.in_unitcode  = 'PaymentAccountsIn'
                          group by dl.in_document, nm.prn, h.rn, sc.faceaccount, h.pref, h.numb, h.doc_date ) iiv
                on iiv.in_document    = pai.h_rn
               and iiv.nomen          = pai.nomen
               and iiv.sc_faceaccount = pai.sc_faceaccount
              /* Приходные ордера */
              left join ( select dl.in_document      
                                ,nm.prn              as nomen
                                ,h.rn                as h_rn
                                ,sc.faceaccount      as sc_faceaccount
                                ,h.indocpref         as head_pref
                                ,h.indocnumb         as head_numb
                                ,h.indocdate         as head_date
                                ,sum(sc.quant_plan)  as quant
                            from inorders         h  
                            join inorderspecs     s   on s.prn  = h.rn
                            join inorderspecsclc  sc  on sc.prn = s.rn
                            join nommodif         nm  on nm.rn  = s.nommodif
                            join doclinks         dl  on dl.out_document = h.rn 
                                                     and dl.in_unitcode  = 'IncomingInvoices'
                          group by dl.in_document, nm.prn, h.rn, sc.faceaccount, h.indocpref, h.indocnumb, h.indocdate ) io
                on io.in_document     = iiv.h_rn
               and io.nomen           = iiv.nomen
               and io.sc_faceaccount  = iiv.sc_faceaccount
--                  ,(select to_date('20.01.2025', 'dd.mm.yyyy') as bg, to_date('06.02.2025', 'dd.mm.yyyy') as en from dual ) d
             where (
               ...
               and nvl(dpos.sumts_double, 'null') != 'Да' 
...
/*    nCountColumn := nCountColumn + 1;
    ...
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.sdlo_ul_auth);
    nCountColumn := nCountColumn + 1;
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.sdlo_pers_auth);
    nCountColumn := nCountColumn + 1;
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
    ...
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.spai_ul_auth);
    nCountColumn := nCountColumn + 1;
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
                                                    ,psvalue => c.spai_ce_auth);
    nCountColumn := nCountColumn + 1;
    udo_pkg_excel_report_xml.p_add_cell_value_string(pnindex => nCountColumn
                                                    ,psstyle => null
